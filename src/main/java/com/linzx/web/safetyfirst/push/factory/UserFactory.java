package com.linzx.web.safetyfirst.push.factory;

import com.google.common.base.Strings;
import com.linzx.web.safetyfirst.push.bean.db.User;
import com.linzx.web.safetyfirst.push.bean.db.UserFollow;
import com.linzx.web.safetyfirst.push.utils.Hib;
import com.linzx.web.safetyfirst.push.utils.TextUtil;
...
public class UserFactory {
	...
	public static User findByName(String name) {
		return Hib.query(session -> (User) session.createQuery("from User where name=:name").setParameter("name", name)
				.uniqueResult());
	}
	...
	public static User bindPushId(User user, String pushId) {
		...
		Hib.queryOnly(session -> {
			...
			List<User> userList = (List<User>) session
					...
					.setParameter("pushId", pushId.toLowerCase()).setParameter("userId", user.getId()).list();
	...
	public static User login(String account, String password) {
		...
		// 寻找
		User user = Hib
				.query(session -> (User) session.createQuery("from User where phone=:phone and password=:password")
						.setParameter("phone", accountStr).setParameter("password", encodePassword).uniqueResult());
	...
	public static UserFollow getUserFollow(final User origin, final User target) {
		return Hib.query(session -> (UserFollow) session
				...
				.setParameter("originId", origin.getId()).setParameter("targetId", target.getId()).setMaxResults(1)
				// 唯一查询返回
				.uniqueResult());
	}
	...
	public static List<User> search(String name) {
		...
		return Hib.query(session -> {
			...
			return (List<User>) session.createQuery(
					...
					.setParameter("name", searchName).setMaxResults(20) // 至多20条
					.list();
